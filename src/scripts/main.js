...
const eventDate = new Date("2024-12-16 19:00:00");
...
const countTime = setInterval(() => {
  ...
  const eventHours = Math.floor(
    (eventTime % daysInMilliseconds) / hoursInMilliseconds,
  );
  const eventMinutes = Math.floor(
    (eventTime % hoursInMilliseconds) / minutesInMilliseconds,
  );
  const eventSeconds = Math.floor((eventTime % minutesInMilliseconds) / 1000);

  document.getElementById("counter").innerHTML =
    `${eventDays}d ${eventHours}h ${eventMinutes}m ${eventSeconds}s`;
});
